# Lab : RAG sur un dépôt de code

## Intention

Tester une chaîne de récupération sur un dépôt logiciel réel.

## Questions à traiter

- Quels documents extraire du dépôt ?
- Comment découper le code sans perdre la structure ?
- Comment évaluer la pertinence des réponses ?

## Checklist qualité

- [ ] Déclic de compréhension identifié.
- [ ] Intention unique.
- [ ] Questions de conception explicites.
- [ ] Concepts reliés au glossaire si nécessaire.
- [ ] Responsabilités ou limites clarifiées.
- [ ] Compromis d'architecture visibles.
- [ ] Chapitre lisible seul.
