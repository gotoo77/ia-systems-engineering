# Glossaire

## Intention

Stabiliser le vocabulaire commun du manuel.

## Termes initiaux

- **LLM** : modèle de langage de grande taille entraîné à prédire et générer du texte.
- **Token** : unité de texte manipulée par un modèle, souvent plus petite qu'un mot.
- **Contexte** : information effectivement visible par le modèle lors d'une inférence.
- **Embedding** : représentation vectorielle d'un contenu pour comparer ou rechercher du sens.
- **RAG** : pattern combinant récupération d'information externe et génération par modèle.
- **Mémoire** : information conservée, récupérée ou reconstruite hors de l'inférence immédiate.
- **Mémoire documentaire** : ensemble de contenus externes pouvant être recherchés ou sélectionnés pour alimenter le contexte.
- **Récupération** : sélection d'informations pertinentes avant une génération.
- **Agent** : système qui choisit des actions, utilise des outils et poursuit un objectif.
- **Orchestrateur** : composant qui coordonne modèles, outils, mémoire, règles et flux.
- **MCP** : protocole de connexion entre modèles, outils et sources de contexte.
- **tree-sitter** : parseur incrémental utilisé pour analyser la structure de code.
- **Quantification** : réduction de précision numérique d'un modèle pour optimiser coût et mémoire.
- **Inférence** : exécution d'un modèle pour produire une sortie à partir d'une entrée.
- **Hallucination** : production plausible mais fausse, non fondée ou non vérifiée.
- **Benchmark** : évaluation comparative selon une tâche, un protocole et des métriques.

## Questions à traiter

- Quels termes doivent être définis avant leur usage ?
- Quels termes évoluent selon le contexte système ?
- Quels termes nécessitent des exemples courts ?
